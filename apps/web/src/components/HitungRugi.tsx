...
import type { IstilahHitung } from "@/lib/jualan";
 ...
 *
 * The words and the value range come from the business-type page
 * (`lib/jualan.ts`): a clinic loses visits, a dealer loses transactions worth
 * far more than a shop order, so one fixed "Rp 2.000.000 max" slider would be
 * useless on half the pages.
 */
export function HitungRugi({
  hargaStarter,
  keDaftar,
  istilah,
}: {
  hargaStarter: number;
  keDaftar: string;
  istilah: IstilahHitung;
}) {
  const [chat, setChat] = useState(20);
  const [luarJam, setLuarJam] = useState(30);
  const [jadiBeli, setJadiBeli] = useState(istilah.jadiAwal);
  const [belanja, setBelanja] = useState(istilah.nilaiAwal);
  ...
  const geser = [
    ...
    { label: istilah.jadi, nilai: jadiBeli, set: setJadiBeli, min: 1, maks: 50, langkah: 1, tampil: `${jadiBeli}%` },
    { label: istilah.nilai, nilai: belanja, set: setBelanja, min: istilah.langkahNilai, maks: istilah.nilaiMaks, langkah: istilah.langkahNilai, tampil: rupiah(belanja) },
  ];
  ...
  return (
    <div className="mx-auto grid max-w-5xl overflow-hidden rounded-3xl border border-ink-200 bg-white shadow-[0_1px_2px_rgba(15,15,15,0.04)] lg:grid-cols-[1.1fr_1fr]">
      <div className="space-y-6 p-5 sm:p-8">
        <p className="text-xs text-ink-500">Angka awalnya cuma contoh. Geser pakai angka {istilah.usaha} sendiri.</p>
        {geser.map((g) => (
      ...
      <div className="flex flex-col justify-between gap-6 bg-ink-950 p-5 text-white sm:p-8">
        <div>
          <p className="text-sm text-white/60">Chat yang masuk di luar jam buka</p>
          <p className="mt-1 text-2xl font-bold tabular-nums">{chatLuarJam.toLocaleString("id-ID")} chat / bulan</p>

          <p className="mt-6 text-sm text-white/60">Yang bisa jadi {istilah.satuan} kalau semuanya langsung dibales</p>
          <p className="mt-1 text-[40px] font-bold leading-tight tracking-[-0.02em] tabular-nums sm:text-[48px]">{rupiah(uang)}</p>
          <p className="text-sm text-white/60">tiap bulan, dari sekitar {order.toLocaleString("id-ID")} {istilah.satuan}</p>

          <p className="mt-6 rounded-2xl bg-white/[0.07] px-4 py-3 text-sm leading-relaxed text-white/85">
            Paket Starter {rupiah(hargaStarter)} sebulan udah balik modal dari <b className="text-white">{balikModal} {istilah.satuan}</b>.
          </p>
        </div>
