        ...
        >
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col sm:flex-row">
            ...
            {/* Pratinjaunya per kotak, bukan satu gumpalan teks. Yang mau
                ...
                menjawab itu. */}
            {/* Satu panel gelap, sama seperti pratinjau contoh Info bisnis.
                Dua layar yang mengerjakan hal yang sama persis tidak boleh
                terlihat seperti dua produk: latar terang di sini dan latar
                gelap di sana bikin orang mengira yang satu kolom isian yang
                bisa diketik dan yang satu cuma bacaan. */}
            <div className="min-h-0 min-w-0 flex-1 p-3 sm:p-4">
              <div className="thin-scroll h-full overflow-auto rounded-xl bg-ink-950 p-4">
                {isian.map(([id, teks], i) => (
                  <div key={id} className={i > 0 ? "mt-5" : ""}>
                    <p className="font-mono text-[11px] uppercase tracking-wide text-ink-500">
                      {NAMA_KOTAK[id] ?? id}
                    </p>
                    <pre className="mt-1 whitespace-pre-wrap break-words font-mono text-[12px] leading-[1.6] text-ink-300">
                      {teks}
                    </pre>
                  </div>
                ))}
