...
const FEATURES: {
  ...
  },
  {
    ...
    // "Ini yang paling sering nambah closing" dibuang: belum ada data pelanggan
    // yang membuktikannya, dan klaim tanpa bukti di halaman ini dilarang.
    body: "Yang nanya terus diem aja disapa ulang otomatis, pakai kalimat yang kamu atur.",
    fitur: "sapaOtomatis",
  },
  {
...
/** Langkah keduanya ikut bidang usaha: klinik mengisi tarif, dealer mengisi skema DP. */
function langkahUntuk(isi: IsiJualan) {
  return [
  {
    ikon: "qr" as NamaIkon,
  ...
  },
  {
    ...
    body: isi.langkahInfo.body,
    pendek: isi.langkahInfo.pendek,
  },
  {
    ikon: "chat" as NamaIkon,
 ...
 */
const TANYA_UMUM: { t: string; j: string }[] = [
  {
 ...
 */
function sorotanUntuk(isi: IsiJualan): IsiSorotan[] {
  return [
    {
      ikon: "chat",
    ...
    { ikon: "info", tab: "Jawab dari data kamu", baris: isi.sorotan.info },
    { ikon: "kalender", tab: isi.sorotan.janjiTab, baris: isi.sorotan.janji },
    { ikon: "sapa", tab: isi.sorotan.sapaTab, baris: isi.sorotan.sapa },
  ];
}
...
const BIDANG = PRESET.filter((p) => p.diHalamanDepan).map((p) => ({
  ...
  nama: p.nama,
  contoh: p.contoh,
  // Keping yang punya halaman sendiri jadi tautan ke sana.
  href: halamanPreset(p.id),
}));
...
export function HalamanJualan({ isi }: { isi: IsiJualan }) {
  ...
  // Pertanyaan khas bidangnya duluan, baru yang berlaku untuk semua usaha.
  const TANYA_JAWAB = [...isi.tanyaJawab, ...TANYA_UMUM];
  const SOROTAN = sorotanUntuk(isi);
  const LANGKAH = langkahUntuk(isi);
    ...
    <main className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-40 border-b border-ink-200/70 bg-white/85 backdrop-blur-md">
        <div className={`${KOLOM} flex h-14 items-center justify-between sm:h-16`}>
          ...
          <nav className="hidden items-center gap-1 text-sm lg:flex">
            ...
            <a href="#bidang" className="rounded-lg px-3 py-2 text-ink-600 transition hover:bg-ink-50 hover:text-ink-900">
              Bidang usaha
            </a>
      ...
      <section className="relative overflow-hidden border-b border-ink-200 bg-white">
        ...
        >
          <div className="mx-auto max-w-4xl text-center">
            ...
            {/* Kalimatnya dari lib/jualan.ts, satu per bidang usaha. Selftest
                memeriksa versi umumnya di sana. */}
            <span className="badge border border-ink-200 bg-white px-3 py-1 text-ink-700">{isi.hero.lencana} · mulai {formatIDR(PLANS.starter.pricePerMonth)}/bulan</span>
            <h1 className="mt-4 text-balance text-[30px] font-bold leading-[1.14] tracking-[-0.03em] text-ink-950 sm:mt-6 sm:text-[46px] lg:text-[54px]">
              {isi.hero.judul}<br className="hidden sm:inline" /> <span className="text-ink-500">{isi.hero.judulAbu}</span>
            </h1>
      ...
      <section id="hitung" className={`scroll-mt-16 sm:scroll-mt-20 ${KOLOM} ${JARAK}`}>
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="judul-bagian mt-3">Berapa {isi.hitung.satuan} yang lepas tiap bulan?</h2>
          <p className="teks-bagian mt-3">Pakai angka {isi.hitung.usaha}. Nggak ada yang dikirim ke mana-mana.</p>
        </div>
        <div className="saat-terlihat mt-9 sm:mt-12">
          <HitungRugi hargaStarter={PLANS.starter.pricePerMonth} keDaftar={keApp("/daftar")} istilah={isi.hitung} />
        </div>
      </section>
        ...
        <div className={`${KOLOM} ${JARAK}`}>
          ...
          <div className="mt-9 sm:mt-12">
            <SorotanTab
              isi={SOROTAN}
              panel={[
                <MockupRasa key="rasa" rasa={isi.rasa} />,
                <div
                  key="info"
                  className="w-full max-w-2xl rounded-2xl border border-ink-200 bg-white p-1.5 bayangan-produk sm:p-2"
                >
                  <MockupInfoBisnis sumber={isi.info} />
                </div>,
                <div
                  key="janji"
                  className="w-full max-w-2xl rounded-2xl border border-ink-200 bg-white p-1.5 bayangan-produk sm:p-2"
                >
                  <MockupJanji janji={isi.janji} />
                </div>,
                <div
                  key="sapa"
                  className="w-full max-w-2xl rounded-2xl border border-ink-200 bg-white p-1.5 bayangan-produk sm:p-2"
                >
                  <MockupSapaLagi sapa={isi.sapa} />
                </div>,
              ]}
            />
          </div>
      ...
      <section id="fitur" className="scroll-mt-16 bg-white sm:scroll-mt-20">
        <div className={`${KOLOM} ${JARAK}`}>
          ...
          {/* Keping yang punya halaman bidang jadi TAUTAN ke halaman itu, dan
              keping halaman yang sedang dibuka dihitamkan. Yang belum punya
              halaman tetap keping biasa: menautkannya ke halaman umum berarti
              mengirim orang ke tempat yang baru saja dia baca. */}
          <p
            id="bidang"
            className="mt-12 scroll-mt-20 text-center text-sm font-medium text-ink-500 sm:mt-16"
          >
            {isi.presetId
              ? "Lihat juga halaman untuk bidang usaha lain"
          ...
          <div className="thin-scroll -mx-5 mt-4 flex gap-2 overflow-x-auto px-5 pb-1 sm:mx-0 sm:flex-wrap sm:justify-center sm:px-0">
            {BIDANG.map((b) => {
              const kelas =
                "flex shrink-0 items-center gap-2 whitespace-nowrap rounded-full border px-3.5 py-2 text-[13px]";
              if (!b.href) {
                return (
                  <span key={b.nama} className={`${kelas} border-ink-200 bg-white text-ink-700`}>
              ...
              const iniHalamannya = b.href === `/${isi.id}`;
              return (
                <Link
                  key={b.nama}
                  href={b.href}
                  aria-current={iniHalamannya ? "page" : undefined}
                  className={`${kelas} tap-aman transition ${
                    iniHalamannya
                      ? "border-ink-900 bg-ink-900 text-white"
                      : "border-ink-200 bg-white text-ink-700 hover:border-ink-400 hover:text-ink-950"
                  }`}
                >
                  <span className={iniHalamannya ? "text-ink-300" : "text-ink-400"}>
                    <Ikon nama={b.ikon} size={15} />
                  </span>
                  {b.nama}
                </Link>
